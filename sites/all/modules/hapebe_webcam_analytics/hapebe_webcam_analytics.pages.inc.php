...
function hapebe_webcam_analytics_page_process($arg0 = FALSE, $arg1 = FALSE, $arg2 = FALSE) {
	...
	if ($arg0 == 'import') {
		...
		$retval[] = hapebe_webcam_analytics_page_maintenance();
		$addTimeStats = TRUE;
 ...
 */
function hapebe_webcam_analytics_page_maintenance() {
	$CFG = hapebe_webcam_analytics_config();
	$log = fopen($CFG['FILE_BASE_PATH'].'/maintenance.log', 'a');
	...
	foreach ($uids as $uid) {
		...
		if (!file_exists($filename)) {
			...
			// remove variables:
			db_delete('vars')->condition('imageid', $img->uid)->execute();
			// remove from sets:
	...
	if (($arg0 == 2) || ($arg0 == 3)) {
		...
		foreach ($lines as $line) {
			...
			for ($i=0; $i<count($tokens); $i++) {
				...
				if ($arg0 == 2) {
					// set variable?
					if (false && in_array($varname, array_keys($img->vars))) {
		...
		if ($arg0 == 2) {
			...
			$retval[] = 
			'<p>'
			.t('Set %nSet variable values, kept %nKeep pre-existing values, encountered %nNotFound unknown IDs.',
				array('%nSet' => $nSet, '%nKeep' => $nKeep, '%nNotFound' => $nNotFound)
			)
			.'</p>';
		}
		if ($arg0 == 3) {
...
function hapebe_webcam_analytics_page_create_set($name) {
	...
	if (($name == 'MINI') || ($name == 'A1000S') || ($name == '7DAY') || ($name == 'YAD7')) {
		$sql = 'SELECT sha256, unixtime FROM images ORDER BY sha256 ASC LIMIT 0,'.$targetCount;
		if ($name == 'YAD7') $sql = 'SELECT sha256, unixtime FROM images ORDER BY sha256 DESC LIMIT 0,'.$targetCount;
...
function hapebe_webcam_analytics_page_export_csv($mode = FALSE) {
	...
	foreach ($set->entries as $sha256) {
		...
		$result = db_query('SELECT varname, number, alpha FROM vars WHERE imageid=:imageid', array(':imageid' => $img->uid));
		foreach ($result as $record) {
			if (in_array($record->varname, $varList)) {
				$values[$record->varname] = empty($record->alpha) ? $record->number : $record->alpha;
			} else {
				$retval[] = 'Not in var list: '.$record->varname;
			}
		}
